<script module lang="ts">
  import type { Assign, HTMLProps, PolymorphicProps } from '$lib/types'

  export interface TreeBranchTextBaseProps extends PolymorphicProps<'span'> {}
  export interface TreeBranchTextProps extends Assign<HTMLProps<'span'>, TreeBranchTextBaseProps> {}
</script>

<script lang="ts">
  import { mergeProps } from '@destyler/svelte'
  import { UI } from '../../factory'
  import { useTreeContext } from '../hooks/use-tree-context'
  import { useTreeNodePropsContext } from '../hooks/use-tree-node-props-context'

  const props: TreeBranchTextProps = $props()

  const tree = useTreeContext()
  const nodeProps = useTreeNodePropsContext()
  const mergedProps = $derived(mergeProps(tree().getBranchTextProps(nodeProps()), props))
</script>

<UI as="span" {...mergedProps} />
