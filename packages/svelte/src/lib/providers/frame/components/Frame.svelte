...
<script lang="ts">
  ...
  import FrameContent from './Content.svelte'
  ...
  let mountNode: HTMLElement | null = $state(null)
  ...
  $effect(() => {
    ...
    mountNode = getMountNode(frameRef)
  })
...
<iframe bind:this={frameRef} bind:this={ref} {...localProps}>
  <EnvironmentProvider value={() => frameRef?.contentDocument ?? document}>
    {#if mountNode}
      <PortalTarget container={mountNode}>
    ...
    {#if mountNode && head && frameRef?.contentDocument?.head}
      <PortalTarget container={frameRef.contentDocument.head}>
        {@render head()}
      </PortalTarget>
    {/if}
  </EnvironmentProvider>
