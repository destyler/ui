...
Where applicable, components are uncontrolled by default but can also be controlled. All behavior wiring is handled internally, so you can get up and running smoothly without creating local states. See [Controllable State](/docs/overview/controllable-state) for `default*` seeds versus live props and `v-model`.
