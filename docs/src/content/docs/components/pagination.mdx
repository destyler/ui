---
title: Pagination
description: A component for navigating between pages of content.
---

import ComponentPreview from '../../../components/ComponentPreview.astro';
import ComponentTypes from '../../../components/ComponentTypes.astro';
import FrameworkContent from '../../../components/FrameworkContent.astro';

> A component for navigating between pages of content.

<ComponentPreview component="pagination" example="Basic" />

## Anatomy

<FrameworkContent fw="vue">
```vue
<script setup lang="ts">
import { Pagination } from '@destyler-ui/vue'
</script>

<template>
  <Pagination.Root>
    <Pagination.PrevTrigger />
    <Pagination.Item />
    <Pagination.Ellipsis />
    <Pagination.NextTrigger />
  </Pagination.Root>
</template>
```
</FrameworkContent>

<FrameworkContent fw="react">
```tsx
import { Pagination } from '@destyler-ui/react'

export default function Basic() {
  return (
    <Pagination.Root>
      <Pagination.PrevTrigger />
      <Pagination.Item />
      <Pagination.Ellipsis />
      <Pagination.NextTrigger />
    </Pagination.Root>
  )
}
```
</FrameworkContent>

<FrameworkContent fw="solid">
```tsx
import { Pagination } from '@destyler-ui/solid'

export default function Basic() {
  return (
    <Pagination.Root count={100}>
      <Pagination.PrevTrigger />
      <Pagination.Item type="page" value={1}>1</Pagination.Item>
      <Pagination.Ellipsis index={1} />
      <Pagination.NextTrigger />
    </Pagination.Root>
  )
}
```
</FrameworkContent>

<FrameworkContent fw="svelte">
```svelte
<script lang="ts">
  import { Pagination } from '@destyler-ui/svelte'
</script>

<Pagination.Root count={100} pageSize={10} siblingCount={2}>
  <Pagination.PrevTrigger>Previous <span class="visually-hidden">Page</span></Pagination.PrevTrigger>
  <Pagination.Context>
    {#snippet render(pagination)}
      {#each pagination().pages as page, index}
        {#if page.type === 'page'}
          <Pagination.Item value={page.value} type={page.type}>{page.value}</Pagination.Item>
        {:else}
          <Pagination.Ellipsis {index}>&#8230;</Pagination.Ellipsis>
        {/if}
      {/each}
    {/snippet}
  </Pagination.Context>
  <Pagination.NextTrigger>Next <span class="visually-hidden">Page</span></Pagination.NextTrigger>
</Pagination.Root>
```
</FrameworkContent>

## Examples

### Basic

<ComponentPreview component="pagination" example="Basic" />

### Controlled

Control the active page of the pagination programmatically.

<ComponentPreview component="pagination" example="Controlled" />

### Initial Page

Seed the starting page with `defaultPage`.

<ComponentPreview component="pagination" example="InitialPage" />

### Customized

Customize the pagination appearance and behavior.

<ComponentPreview component="pagination" example="Customized" />

### Root Provider

<ComponentPreview component="pagination" example="RootProvider" />

## API Reference

<ComponentTypes id="pagination" />
